...
// BuildTx builds a StdSignMsg as per the parameters passed in the contexts.
func BuildTx(txCtx authctx.TxContext, cliCtx context.CLIContext, msgs []sdk.Msg) (auth.StdSignMsg, error) {
	txCtx, err := prepareTxContext(txCtx, cliCtx)
	if err != nil {
		return auth.StdSignMsg{}, err
	}
	return txCtx.Build(msgs)
}

// MarshalJSON builds the transaction and returns its JSON encoding.
func MarshalJSON(txCtx authctx.TxContext, cliCtx context.CLIContext, msgs []sdk.Msg) ([]byte, error) {
	stdSignMsg, err := BuildTx(txCtx, cliCtx, msgs)
	if err != nil {
		return nil, err
	}
	json, err := cliCtx.Codec.MarshalJSON(stdSignMsg)
	if err != nil {
		return nil, err
	}
	return json, nil
